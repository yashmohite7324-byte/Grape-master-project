...
export default function FarmerCartPage() {
  ...
  const [razorpayData, setRazorpayData] = useState<{ isOpen: boolean; orderId: string; orderNumber: string; amount: number }>({
    isOpen: false,
    orderId: '',
    orderNumber: '',
    amount: 0,
  ...
  const placeOrder = async (e: React.FormEvent) => {
    ...
    setPlacing(true)
    try {
      const order = await api.post<{ id: string; orderNumber: string; totalAmount: number }>('/orders', {
        sellerId: items[0].sellerId,
        orderType: 'FERTILIZER',
        fulfillmentMethod: fulfillment,
        guestPhone: address.mobileNumber || '9876543210',
        guestName: address.fullName || 'Farmer Partner',
        shippingAddress: {
          fullName: address.fullName || 'Farmer',
          mobileNumber: address.mobileNumber || '9876543210',
          addressLine: address.addressLine || 'Farm Address',
      ...
      show('success', `Order ${order.data.orderNumber} created! Complete payment below.`)
      setRazorpayData({
        isOpen: true,
        orderId: order.data.id,
        orderNumber: order.data.orderNumber,
        amount: order.data.totalAmount || total,
      })
    } catch (err) {
  ...
  const handlePaymentSuccess = () => {
    clear()
    setRazorpayData(r => ({ ...r, isOpen: false }))
    show('success', 'Payment confirmed! Order sent to seller.')
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-2xl">
        <h1 className="mb-6 font-display text-3xl font-semibold text-ink">Farm Supplies Cart</h1>
        <EmptyState icon={<ShoppingCart className="h-8 w-8" />} title="Your supplies cart is empty"
          action={<Link href="/farmer/marketplace"><Button>Browse Fertilizers & Inputs</Button></Link>} />
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-2xl">
          ...
          <Card className="p-5">
            ...
            <Field label="Fulfillment">
              <Select value={fulfillment} onChange={e => setFulfillment(e.target.value as 'PICKUP' | 'DELIVERY')}>
                ...
                <option value="PICKUP">Dealer pickup</option>
              </Select>
            </Field>
            {fulfillment === 'DELIVERY' && (
              <div className="mt-4 space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Full name"><Input required value={address.fullName} onChange={setAddr('fullName')} placeholder="Full name" /></Field>
                  <Field label="Mobile"><Input required value={address.mobileNumber} onChange={setAddr('mobileNumber')} placeholder="Mobile number" /></Field>
                </div>
                <Field label="Farm Address"><Input required value={address.addressLine} onChange={setAddr('addressLine')} placeholder="Village/Gut No." /></Field>
                <div className="grid gap-4 sm:grid-cols-3">
                  <Field label="Village"><Input required value={address.village} onChange={setAddr('village')} placeholder="Village" /></Field>
                  <Field label="District"><Input required value={address.district} onChange={setAddr('district')} placeholder="District" /></Field>
                  <Field label="State"><Input required value={address.state} onChange={setAddr('state')} placeholder="Maharashtra" /></Field>
                </div>
                <Field label="PIN code"><Input required value={address.pincode} onChange={setAddr('pincode')} placeholder="PIN code" /></Field>
              </div>
            )}
          </Card>
      ...
      <RazorpayModal
        isOpen={razorpayData.isOpen}
        onClose={() => setRazorpayData(r => ({ ...r, isOpen: false }))}
