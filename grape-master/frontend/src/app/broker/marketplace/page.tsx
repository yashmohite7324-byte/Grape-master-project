...
export default function BrokerMarketplacePage() {
  ...
  const listingsList: FarmerListing[] = Array.isArray(data) ? data : ((data as any)?.items ?? (data as any)?.data ?? [])

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-3xl font-semibold text-ink">Farmer Marketplace</h1>
        <span className="text-sm text-muted">{listingsList.length} listings available</span>
      </div>
      ...
      {loading ? <Spinner /> : listingsList.length === 0 ? (
        <EmptyState icon={<Store className="h-8 w-8" />} title="No listings match" hint="Try widening your filters or clearing search terms." />
      ) : (
            ...
            <Link key={l.id} href={`/broker/marketplace/${l.id}`}>
              <Card className="overflow-hidden card-hover group">
                <div className="relative h-44 overflow-hidden">
                  <img src={getCropImage(l.cropType, l.id, '400x200', l.images?.[0])}
