...
export const getMyListings = catchAsync(async (req: Request, res: Response) => {
  ...
  const { items, meta } = await farmerService.getMyListings(req.user!.id, { page, limit }, status)
...
export const getReceivedOffers = catchAsync(async (req: Request, res: Response) => {
  ...
  const { items, meta } = await farmerService.getReceivedOffers(req.user!.id, { page, limit })
