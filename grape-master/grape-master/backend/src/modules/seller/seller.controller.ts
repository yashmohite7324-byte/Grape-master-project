...
export const getMyProducts = catchAsync(async (req: Request, res: Response) => {
  ...
  const { items, meta } = await sellerService.getMyProducts(req.user!.id, { page, limit })
...
export const getMyOrders = catchAsync(async (req: Request, res: Response) => {
  ...
  const { items, meta } = await sellerService.getMyOrders(req.user!.id, { page, limit }, status)
