...
export const browseMarketplace = async (page: PageParams, filters: MarketplaceFilters) => {
  const where: Prisma.FarmerListingWhereInput = {
    ...
    ...(filters.minQuantity ? { availableQuantity: { gte: filters.minQuantity } } : {}),
...
export const makeOffer = async (
  ...
  // Real-time notify the farmer
  notifications.offerReceived(
    listing.farmerId, 'A broker', listing.cropType,
    input.quantity, listing.unit, totalAmount
  )
