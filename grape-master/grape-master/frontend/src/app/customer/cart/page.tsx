...
export default function CartPage() {
  ...
  const [address, setAddress] = useState({
    fullName: '', mobileNumber: '', addressLine: '',
  ...
  const placeOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/payment?orderId=${order.data.id}`)
    } catch (err) {
      ...
      {!checkout ? (
        ...
        <form onSubmit={placeOrder} className="space-y-4">
            ...
            {fulfillment === 'DELIVERY' && (
              <div className="mt-4 space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Full name"><Input required value={address.fullName} onChange={setAddr('fullName')} /></Field>
                  <Field label="Mobile"><Input required value={address.mobileNumber} onChange={setAddr('mobileNumber')} /></Field>
                </div>
                <Field label="Address"><Input required value={address.addressLine} onChange={setAddr('addressLine')} /></Field>
                <div className="grid gap-4 sm:grid-cols-3">
                  <Field label="Village"><Input required value={address.village} onChange={setAddr('village')} /></Field>
                  <Field label="District"><Input required value={address.district} onChange={setAddr('district')} /></Field>
                  <Field label="State"><Input required value={address.state} onChange={setAddr('state')} /></Field>
                </div>
                <Field label="PIN code"><Input required value={address.pincode} onChange={setAddr('pincode')} /></Field>
              </div>
            )}
          </Card>
          ...
          <div className="flex gap-3">
            <Button type="submit" loading={placing} className="flex-1">Place order · {formatINR(total)}</Button>
