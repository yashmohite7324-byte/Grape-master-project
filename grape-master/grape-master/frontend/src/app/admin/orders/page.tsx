            ...
            <tbody className="divide-y divide-line">
              {(data ?? []).map((o: Order) => (
                <tr key={o.id} className="hover:bg-paper/50">
                  ...
                  <td className="px-4 py-3 text-muted">{o.orderType.replace('_', ' ')}</td>
                  <td className="px-4 py-3 text-ink">{o.buyer?.profile?.fullName ?? '—'}</td>
                  <td className="px-4 py-3 text-ink">{o.seller?.profile?.fullName ?? '—'}</td>
                  <td className="px-4 py-3 font-mono tabular-nums text-ink">{formatINR(o.totalAmount)}</td>
                  <td className="px-4 py-3"><Pill className={statusStyle[o.paymentStatus]}>{o.paymentStatus}</Pill></td>
